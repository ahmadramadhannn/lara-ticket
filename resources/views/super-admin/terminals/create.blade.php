    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('super-admin.terminals.store') }}" method="POST" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required
                                   ...
                                   placeholder="e.g. Terminal Kalideres">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="text" name="code" id="code" value="{{ old('code') }}" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                   placeholder="e.g. JKT-KLD" maxlength="20">
                            <p class="mt-1 text-sm text-gray-500">{{ __('Unique code for the terminal (max 20 characters)') }}</p>
                            @error('code')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>

                        <div x-data="{ provinceId: '{{ old('province_id', '') }}', cities: [] }" x-init="
                            if (provinceId) {
                                cities = @js($provinces->keyBy('id')->map(fn($p) => $p->cities)->toArray())[provinceId] || [];
                            }
                        ">
                            ...
                            <select name="province_id" id="province_id" 
                                    x-model="provinceId"
                                    @change="cities = @js($provinces->keyBy('id')->map(fn($p) => $p->cities)->toArray())[provinceId] || []"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <textarea name="address" id="address" rows="3"
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                      placeholder="{{ __('Full address of the terminal') }}">{{ old('address') }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
